// WORLD CUP CLASH v5 — balance pass on the v4 xG engine (GDD v5 §4, §6, §15)
// v5 adds: stamina ramp (8/10/12), per-round card cap (4/5/6), rarity stat multiplier.
...
(function () {
  ...
  const DEF_T = {
    goalsToWin: 3, roundCap: 10, halftimeRound: 5,
    xgFloor: 0.05, xgSlope: 150, xgCap: 0.60,
    openingHand: 5, drawPerRound: 3, handCap: 8,
  ...
  // v5 ramps (GDD §6, §15) — keyed off the (tunable) base values
  function staminaFor(T, round) {
    const base = T.staminaPerRound;
    return round <= 5 ? base : round <= 8 ? base + 2 : base + 4;
  }
  function cardCapFor(T, round) {
  ...
  function newPlayer(cards, captainId, T) {
    ...
    return {
      goals: 0, xg: 0, xgTotal: 0, fatigue: 0, scoredFirstAt: null,
  ...
  // 10-round cap → tiebreak ladder: goals → first scorer → accumulated xG (GDD §14)
  function decideAtCap(state) {
    const [A, B] = state.players;
    if (A.goals !== B.goals) {
      state.winner = A.goals > B.goals ? 0 : 1;
      state.capReason = "more goals at the whistle";
      return;
    }
    const fa = A.scoredFirstAt, fb = B.scoredFirstAt;
    if (fa != null || fb != null) {
      if (fa != null && (fb == null || fa < fb)) { state.winner = 0; state.capReason = "level on goals — you scored first"; return; }
      if (fb != null && (fa == null || fb < fa)) { state.winner = 1; state.capReason = "level on goals — they scored first"; return; }
    }
    if (A.xgTotal !== B.xgTotal) {
      state.winner = A.xgTotal > B.xgTotal ? 0 : 1;
      state.capReason = "0–0 — decided on accumulated xG (" + r2(A.xgTotal) + " vs " + r2(B.xgTotal) + ")";
      return;
    }
    state.winner = "draw";
    state.capReason = "dead level — the shootout went their way";
  }
  ...
  function startRound(state) {
    ...
    state.players.forEach((P, idx) => {
      ...
      // v5: stamina ramps with the match — 8 (R1–5) → 10 (R6–8) → 12 (R9–10) (GDD §6)
      P.maxStamina = staminaFor(T, state.round);
  ...
  function canPlace(state, card, lane) {
    ...
    if (laneCount(P) >= cardCapFor(state.T, state.round)) return false;
  ...
  function playTactic(state, card, opts) {
    ...
    P.hand = P.hand.filter((c) => c !== card);
    if (k === "waterBreak") {
      P.fatigue = 0; P.stamina += 2; P.discard.push(card);
      return true;
    }
    if (k === "substitution") {
  ...
  function reveal(state) {
    ...
    // Halftime — both sides get fresh legs (GDD §8)
    if (state.round === T.halftimeRound) {
    ...
    [0, 1].forEach((p) => {
      ...
      queue.filter((q) => q.p === p).forEach((q) => {
        if (q.card.category === "power" && !q.cancelled && !q.wasted) {
    ...
    // 15. Win check — first to 3 goals (GDD §14)
    const [A, B] = P;
    if (A.goals >= T.goalsToWin || B.goals >= T.goalsToWin) {
      if (A.goals !== B.goals) state.winner = A.goals > B.goals ? 0 : 1;
      else state.winner = (A.scoredFirstAt || 99) <= (B.scoredFirstAt || 99) ? 0 : 1;
    }
  ...
  function aiPlan(state) {
    ...
    const playT = (c) => {
      ...
      if (k === "waterBreak") { P.fatigue = 0; P.stamina += 2; P.discard.push(c); }
    ...
    if (lead < 0 || P.fatigue >= 16 || Y.goals >= T.goalsToWin - 1) form = "offensive";
    ...
    const cap = cardCapFor(T, state.round);
    ...
    P.hand.filter((c) => c.type === "player" && c.position !== "GK")
      ...
      .forEach((c) => {
        if (!atCap() && P.hand.includes(c) && spentA + c.cost <= atkBudget && aff(c)) { placeAI(c, "attack"); spentA += c.cost; }
      });
    P.hand.filter((c) => c.type === "player")
      .sort((a, b) => b.def - a.def)
      .forEach((c) => {
        if (!atCap() && P.hand.includes(c) && aff(c) && P.stamina - c.cost >= reserve) placeAI(c, "defense");
      });
    ...
    if (hog && aff(hog) && !P.hogUsed && gateOkAI("handOfGod") && (Y.goals >= T.goalsToWin - 1 || (lead < 0 && state.round >= 6))) playT(hog);
  ...
  window.WCC5E = Object.assign({}, window.WCC3E, {
