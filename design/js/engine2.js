// WORLD CUP CLASH v2 — Slay-the-Spire-style engine (pure logic, no UI)
(function () {
  const D = () => window.WCC2;
  ...
  const DEF_T = {
    startMorale: 100, openingHand: 5, drawPerRound: 3, handCap: 8,
    staminaBase: 3, staminaCap: 8, slotBudget: 20, deckSize: 30,
    stoppageAt: 20, stoppageBonus: 3, momentumBonus: 6,
  ...
  // ---------- match / round ----------
  function newMatch(playerCards, captainId, tuning) {
  ...
  function startRound(state) {
    ...
    state.players.forEach((P, idx) => {
      ...
      P.maxStamina = Math.min(T.staminaCap, state.round + T.staminaBase);
  ...
  function reveal(state) {
    ...
    [0, 1].forEach((p) => {
      ...
      if (live(p, "counterAttack").length && tot[p].def >= tot[o].atk) {
        const s = tot[p].def - tot[o].atk;
        if (s > 0) hit(o, s, "Counter-attack on the break");
  ...
  function aiPlan(state) {
    ...
    // 4. Mode: defensive when hurting, all-in when you're nearly broken
    const mode = P.morale <= 30 ? 0.35 : Y.morale <= 25 ? 0.8 : 0.55;
    ...
    // 7. Finishers & situational tactics
    const pen = find("penalty");
    if (pen && aff(pen) && P.board.attack.some((c) => c.position === "FWD") && (Y.morale <= 20 || state.round >= 7)) playT(pen);
    const lb = find("longBall");
    if (lb && aff(lb) && Y.morale <= 12 && P.hand.filter((c) => c.type === "player").length >= 2) playT(lb);
    const nutT = find("nutmeg");
    if (nutT && aff(nutT) && P.board.attack.some((c) => c.position === "FWD" && c.atk >= 85)) playT(nutT);
    const hog = find("handOfGod");
    if (hog && aff(hog) && !P.hogUsed && Y.morale <= 30 && P.board.attack.length >= 2) playT(hog);
    ...
    state.aiIntent = { cards: laneCards(P).length + P.tactics.length, stamina: spent };
  }
  ...
  function aiSquad() {
    const stars = [
      byName("Kylian Mbappé", 2026), byName("Ousmane Dembélé", 2026), byName("N'Golo Kanté", 2018),
      byName("Mike Maignan", 2026), byName("William Saliba", 2026), byName("Eduardo Camavinga", 2026),
    ].filter(Boolean);
    const tactics = ["offsideTrap", "referee", "counterAttack", "catenaccio", "waterBreak", "highPress", "penalty"]
      .map(tByKind).filter(Boolean);
    const commons = [
      ["Maya Yoshida", 2022], ["Ko Itakura", 2026], ["Hidemasa Morita", 2026], ["Ao Tanaka", 2026],
      ["Nahitan Nández", 2022], ["Sebastián Coates", 2022], ["Mathías Olivera", 2026], ["Guillermo Varela", 2022],
      ["Nayef Aguerd", 2022], ["Azzedine Ounahi", 2022], ["Jawad El Yamiq", 2022],
      ["Wout Weghorst", 2022], ["Davy Klaassen", 2022], ["Marten de Roon", 2022],
      ["Milan Borjan", 2022], ["Hwang Hee-chan", 2026], ["Kim Young-gwon", 2022],
    ].map(([n, y]) => byName(n, y)).filter(Boolean);
    const cards = [...stars, ...tactics, ...commons];
    return { cards, captainId: stars[0].id };
  }

  function presetSquad() {
    const stars = [
      byName("Lionel Messi", 2022), byName("Julián Alvarez", 2026), byName("Lautaro Martínez", 2026),
      byName("Alexis Mac Allister", 2026), byName("Emiliano Martínez", 2026),
    ].filter(Boolean);
    const tactics = ["waterBreak", "counterAttack", "offsideTrap", "referee", "penalty", "tikiTaka", "catenaccio", "substitution"]
      .map(tByKind).filter(Boolean);
    const commons = [
      ["Héctor Herrera", 2022], ["Luis Chávez", 2022], ["César Montes", 2026], ["Alexis Vega", 2026], ["Jesús Gallardo", 2022],
      ["Walker Zimmerman", 2022], ["Kellyn Acosta", 2022], ["Cristian Roldan", 2026], ["Sean Johnson", 2022],
      ["Ben Davies", 2022], ["Harry Souttar", 2022], ["Jackson Irvine", 2022], ["Joe Allen", 2022],
      ["Kamal Miller", 2022], ["Cyle Larin", 2022], ["Tajon Buchanan", 2026], ["Alistair Johnston", 2026],
    ].map(([n, y]) => byName(n, y)).filter(Boolean);
    const cards = [...stars, ...tactics, ...commons];
    return { cards, captainId: stars[0].id };
  }
  ...
  window.WCC2E = {
    ...
    canPlace, place, recall, playTactic,
    presetSquad, aiSquad, effStats, chemNations, status,
  };
})();
