// WORLD CUP CLASH v8 — the 90-minute match (GDD v8 §6, §8, §10, §12, §14, §15)
// v8 changes vs v7 (hand & tactical limits):
//   • MINIMUM HAND OF 5 — draw back UP TO 5 each round (refill to the 5-card minimum) instead of a
//     flat 3; grays reshuffle mid-draw so a thin deck never strands you on a 2-card hand. (Old draw-3/cap-8 gone.)
//   • TACTICAL PLAY CAP — at most 2 Tactical Cards per half, 4 per match; reset at halftime / ET.
//   • (Run tactical deck cap ~4 is handled in the locker as a swap — see Run8/App8.)
// v7 carried over: full 90' → 3-goal MERCY or most goals at full time; golden-goal EXTRA TIME (xG ×2,
//   meters reset); premium players LOCK until halftime / ET; Tactical Cards single-use (EXILED).
...
(function () {
  ...
  function playTactic(state, card, opts) {
    ...
    if (k === "waterBreak") {
      P.fatigue = 0; P.stamina += 2; P.exiled.push(card);
      return true;
    }
    if (k === "substitution") {
  ...
  function aiPlan(state) {
    ...
    const playT = (c) => {
      ...
      if (k === "waterBreak") { P.fatigue = 0; P.stamina += 2; P.exiled.push(c); }
  ...
  window.WCC8E = Object.assign({}, window.WCC3E, {
