...
function Locker3({ run, beaten, reward, onContinue }) {
  ...
  return (
    <div className="screen locker" data-screen-label="locker-room">
      ...
      <div className="locker-body">
          ...
          <div className="offer-row">
            {reward.offers.map((c) => (
              <div
                ...
                onClick={() => { setChosenId(chosenId === c.id ? null : c.id); setSkipped(false); }}
              >
                {chosenId === c.id && <span className="pick-tag">TAKING THIS</span>}
                <TCard card={c} size={158} showSlots />
              </div>
            ))}
          </div>
          <div className="note3">A thicker deck means more tools but less consistency — skipping keeps your XI coming back around faster.</div>
          <div style={{ display: "flex", gap: 10 }}>
            <button className={`btn ${skipped ? "btn-primary" : "btn-ghost"}`} style={{ fontSize: 13, padding: "9px 16px" }}
              onClick={() => { setSkipped(!skipped); setChosenId(null); }}>
        ...
        <div className="locker-col">
          <h4>Your deck · {run.deck.length + (chosen ? 1 : 0)} cards</h4>
          ...
          <div className="pick-rows" style={{ overflowY: "auto", flex: 1, minHeight: 0 }}>
            ...
            {(tacticals.length > 0 || chosen) && <div className="group-h">Tactical cards · {tacticals.length + (chosen ? 1 : 0)}</div>}
            {tacticals.map((c) => (
              <div key={c.id} className="pick-row tactic-row">
                <span className="rt">T</span>
                <span className="nm">{c.name}</span>
            ...
            {chosen && (
              <div className="pick-row tactic-row" style={{ borderColor: "var(--gold)" }}>
                <span className="rt">T</span>
                <span className="nm">{chosen.name} — new</span>
              </div>
            )}
          </div>
          <button className="btn btn-gold btn-big" disabled={!decided} onClick={() => onContinue(chosen, captainId)}>
            {decided ? "On to the " + nextStage.label + " →" : "Choose a tactical card or skip"}
          </button>
        </div>
      </div>
