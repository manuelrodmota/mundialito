// WORLD CUP CLASH v7 — run shell: the 90-minute match (mercy, golden-goal ET, locked stars, single-use Tacticals)
...
function HowToPlay5({ onClose, t }) {
  const rows = [
    ...
    ["Extra time", `Level at 90'? Golden goal: meters reset to 0, your benched stars and fatigue refresh, and every xG counts double. Next goal wins.`],
    ["Star quality", "Rarer cards hit harder: each contributes its stats × a rarity multiplier (rare ×1.1, epic ×1.2, legendary ×1.3) — but a played star benches until halftime, so commons are your sustain engine."],
    ["Single-use tactics", "Tactical cards are face-up the moment they're played, by both sides — and they're spent for good (exiled) once used. Your 1 tactical is a precious one-shot trump."],
    ["Fatigue", "You can't park the bus forever. Defending tires your back line, attacking rests it — fatigue saps your effective DEF, so the opponent's meter fills faster. Halftime (45') gives both sides fresh legs and returns benched stars; Water Break clears yours."],
    ["Plan", `Draw 3 each round. Stamina ramps as the match heats up — ${t.staminaPerRound} (R1–5), ${t.staminaPerRound + 2} (R6–8), ${t.staminaPerRound + 4} (R9–10) — and you may field at most ${t.cardCapBase} players a round (rising to ${t.cardCapBase + 1}, then ${t.cardCapBase + 2}). Pick a formation, secretly field players into ATTACK or DEFENSE.`],
