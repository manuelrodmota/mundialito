// WORLD CUP CLASH v9 — XI builder. v9: the slot budget buys a PREMIUM core; commons are NOT
// hand-pickable — “Fill bench” rolls RANDOM commons to complete the 11 (re-click to re-roll).
function Builder9({ slotBudget, onStart, onBack }) {
  ...
  const filtered = PLAYERS.filter((c) =>
        c.slots > 0 &&                                  // v9: commons aren't pickable — premiums only
  ...
  const togglePlayer = (card) => {
    if (card.slots === 0) return;   // v9: commons can't be hand-picked
    if (inPicks(card)) {
      setPicks(picks.filter((c) => c.id !== card.id));
      if (captainId === card.id) setCaptainId(null);
    } else {
      const premiums = picks.filter((c) => c.slots > 0);
      if (premiums.length >= XI) return;
      if (slotsUsed + card.slots > slotBudget) return;
      // keep premiums + this card, then trim the rolled commons so the roster never exceeds 11
      const commons = picks.filter((c) => c.slots === 0);
      const room = XI - (premiums.length + 1);
      setPicks([...premiums, card, ...commons.slice(0, Math.max(0, room))]);
  ...
  // v9: roll RANDOM commons to complete the 11 (premiums kept; re-click to re-roll the bench).
  const fillBench = () => {
    const premiums = picks.filter((c) => c.slots > 0);
    const room = XI - premiums.length;
    if (room <= 0) { setPicks(premiums); return; }
    const pool = PLAYERS.filter((c) => c.slots === 0);
    const shuffled = pool.slice();
    for (let i = shuffled.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
    }
    setPicks([...premiums, ...shuffled.slice(0, room)]);
  };
  ...
  const groups = [
    ...
    ["Bench · random commons", picks.filter((c) => c.rarity === "common")],
  ];
    ...
    <div className="screen builder" data-screen-label="xi-builder">
      ...
      <div className="builder-head">
        <div>
          ...
          <div className="hint">Spend your {slotBudget}-slot budget on a premium core (rares, epics, legendaries), plus at most 1 tactical card. The rest of the 11 fills with <b>random commons</b> — you can’t hand-pick them. Crown a captain.</div>
        </div>
      ...
      <div className="builder-body">
        ...
        <div className="picks-pane" style={{ width: 340 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button className="btn btn-ghost" style={{ flex: 1, padding: "9px 10px", fontSize: 13 }} onClick={fillBench}>
              Fill bench (random)
            </button>
