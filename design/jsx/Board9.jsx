// WORLD CUP CLASH v10 — match board: numeric scoreboard + match clock, 3-goal mercy, golden-goal
// extra time, bench (locked premium players) + exiled (single-use Tactical) piles.
// v10 balance pass: per-round field cost is the gentle rarity curve and a STAR-CORE DISCOUNT halves
// support cards in a lane that holds a premium — so the remaining-stamina readout is derived from the
// committed lanes, and a “star core” chip lights up when the discount is live. Engine = WCC9E.

function Board9({ squad, captainId, team, stage, tuning, motion, showIntentFormation, onWin, onRunOver, onConcede }) {
  const E = window.WCC9E;
  ...
  const committed = laneCardsOf(me).length + me.tactics.length;
  function laneCardsOf(P) { return [...P.board.attack, ...P.board.defense]; }
  // v10: a lane holding a premium gives every other card in it the star-core half-price discount
  const starCoreLive = me.board.attack.some((c) => c.rarity !== "common" && me.board.attack.length > 1)
    || me.board.defense.some((c) => c.rarity !== "common" && me.board.defense.length > 1);
  ...
  return (
    <div className={`screen board v4board ${shake ? "shake" : ""} ${m.extraTime ? "et-mode" : ""}`} data-screen-label={"match-" + stage.key}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "6px 26px", position: "relative", zIndex: 5, minHeight: 56 }}>
        ...
        {starCoreLive && (
          <span className="cap-chip5 star" title="Star core active — in a lane holding a premium player, the costliest card pays full and the rest are half-price (min 1).">
            ★ star core · support half-price
          </span>
        )}
        <FormationPicker3
          value={me.formation}
      ...
      <div className="hand-dock" style={{ "--hw": hw + "px" }}>
        ...
        <div className="fan2">
          {me.hand.map((c, idx) => {
            const afford = (c.type === "tactic"
              ? me.stamina >= c.cost
              : me.stamina >= Math.min(
                  c.position === "GK" ? Infinity : E.marginalCost(me, c, "attack"),
                  E.marginalCost(me, c, "defense")
                )
            ) && !(c.type === "tactic" && c.effect.kind === "handOfGod" && me.hogUsed);
