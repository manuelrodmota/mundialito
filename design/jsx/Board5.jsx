// WORLD CUP CLASH v5 — match board: xG meters, goals, fatigue, visible tacticals + v5 balance pass
// (stamina ramp, per-round card cap, rarity stat multiplier). Widgets in Board4Widgets.jsx.

function Board5({ squad, captainId, team, stage, tuning, motion, showIntentFormation, onWin, onRunOver, onConcede }) {
  const E = window.WCC5E;
  const FM = window.WCC3.FORMATION_META;
  const matchRef = useRef(null);
  if (!matchRef.current) matchRef.current = E.newMatch(squad, captainId, team, tuning);
  const m = matchRef.current;
  useEffect(() => { window.__match5 = m; });
  const T = m.T;
  ...
  const cardCap = E.cardCapFor(T, m.round);
  ...
  return (
    <div className={`screen board v4board ${shake ? "shake" : ""}`} data-screen-label={"match-" + stage.key}>
      ...
      <div className="side-strip top">
        ...
        <CenterBoard5 team={team} sMe={sMe} sAi={sAi} T={T} round={m.round} shake={shake} />
      </div>
      ...
      <div className="hand-dock" style={{ "--hw": hw + "px" }}>
        <DeckPile5 kind="draw" count={me.draw.length} w={Math.round(hw * 0.62)} pileRef={drawPileRef} pulse={drawPulse} />
        <div className="fan2">
          {me.hand.map((c, idx) => {
        ...
        <DeckPile5 kind="discard" count={me.discard.length} w={Math.round(hw * 0.62)} pileRef={discardPileRef} pulse={discardPulse} />
      </div>
