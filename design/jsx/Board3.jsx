// WORLD CUP CLASH v3 — match board: lanes + formations + historic opponent
function Pips3({ cur, max }) {
  return (
...
function Morale3({ P, name, T, shownMorale }) {
...
function PileChips3({ P, stoppage }) {
  return (
...
function PowerShelf3({ P, onInspect }) {
...
function Lane3({ title, kind, cards, mine, droppable, rejecting, faceDown, owner, state, lw, onZoneClick, onCardClick, onInspect }) {
...
function Clash3({ show, atk, def, mineAttacking, dmg }) {
...
function FormationPicker3({ value, onChange, swing, disabled }) {
  const F = window.WCC3.FORMATION_META;
  const m = (1 + swing / 100).toFixed(2);
  const w = (1 - swing / 100).toFixed(2);
  const rows = [
    ["offensive", `ATK ×${m} · DEF ×${w}`],
    ["balanced", "ATK ×1 · DEF ×1"],
    ["defensive", `ATK ×${w} · DEF ×${m}`],
  ];
  return (
    <div className="formation-picker">
      <span className="fp-label">Formation</span>
      {rows.map(([id, eff]) => (
        <button key={id} className={value === id ? "on" : ""} data-f={id} disabled={disabled} onClick={() => onChange(id)}>
          <b>{F[id].shape}</b>
...
function Board3({ squad, captainId, team, stage, tuning, motion, showIntentFormation, onWin, onRunOver, onConcede }) {
  const E = window.WCC3E;
  const FM = window.WCC3.FORMATION_META;
  const matchRef = useRef(null);
  if (!matchRef.current) matchRef.current = E.newMatch(squad, captainId, team, tuning);
  const m = matchRef.current;
  useEffect(() => { window.__match3 = m; });
  const T = m.T;
  ...
  const [stageAnim, setStageAnim] = useState(null); // null | preflip | flip | slam | after | readout
  ...
  const showTotals = stageAnim === "slam" || stageAnim === "after" || stageAnim === "readout";
  const showReadout = stageAnim === "readout";
  const showDmg = stageAnim === "after" || stageAnim === "readout";
  ...
  return (
    <div className={`screen board v2board ${shake ? "shake" : ""}`} data-screen-label={"match-" + stage.key}>
      ...
      <div className="side-strip top">
        ...
        <Morale3 P={ai} name="" T={T} shownMorale={shownAi} />
        ...
        <PileChips3 P={ai} stoppage={m.stoppage} />
      </div>
      ...
      <div className="zones">
        {planning && m.aiIntent && T.intent && (
          <div className="intent">
            Opponent committed <b>{m.aiIntent.cards}</b> card{m.aiIntent.cards === 1 ? "" : "s"} · <b>{m.aiIntent.stamina}</b> stamina
            {showIntentFormation && (
              <React.Fragment> · <b>{FM[m.aiIntent.formation].label} ({FM[m.aiIntent.formation].shape})</b></React.Fragment>
            )}
          </div>
        )}
        <div className="zcol">
          <Lane3 title="Their defense" kind="def" cards={lanesAi.defense} faceDown={aiFaceDown} owner={1} state={m} lw={lw} onInspect={setInspect} />
          <Clash3 show={showTotals && tot} atk={tot ? tot[0].atk : 0} def={tot ? tot[1].def : 0} mineAttacking={true} dmg={tot ? Math.max(0, tot[0].atk - tot[1].def) : 0} />
          <Lane3
        ...
        <div className="zcol">
          <Lane3 title="Their attack" kind="atk" cards={lanesAi.attack} faceDown={aiFaceDown} owner={1} state={m} lw={lw} onInspect={setInspect} />
          <Clash3 show={showTotals && tot} atk={tot ? tot[1].atk : 0} def={tot ? tot[0].def : 0} mineAttacking={false} dmg={tot ? Math.max(0, tot[1].atk - tot[0].def) : 0} />
          <Lane3
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "6px 26px", position: "relative", zIndex: 5, minHeight: 56 }}>
        ...
        {(planning ? me.tactics : (m.lastBoards ? m.lastBoards[0].tactics.map((t) => t.card) : [])).length > 0 && (
          <div className="shelf">
            <span className="label">Your tactical cards</span>
      ...
      <div className="side-strip bottom">
        ...
        <Morale3 P={me} name="Your XI" T={T} shownMorale={shownMe} />
        ...
        <PileChips3 P={me} stoppage={m.stoppage} />
      ...
      {ended && (
        <div className="overlay">
          ...
          <div className="note">
            {won
              ? `${team.name} broke after ${m.round} rounds. You dealt ${me.dealtTotal} total damage.`
              : m.winner === "draw"
                ? `25 rounds couldn't separate you — and the shootout went their way. The run is over.`
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {won ? (
              <button className="btn btn-gold btn-big" onClick={() => onWin({ rounds: m.round, dealt: me.dealtTotal, mvp })}>
